...
void input_control::handle_inputs(){
	if(app.eng.gd.players.size() <= 0){ return; }

	int max_speed = (app.eng.superfast ? 20 : 5);

	if(keys_down[VK_UP]){
		app.eng.gd.players[0]->speed = max_speed;
	...
	if(app.eng.js.number_of_joysticks > 0){
		for(int player_num = 0; player_num < app.eng.number_of_players; player_num++){
			if(!(player_num < app.eng.js.number_of_joysticks))
				continue;

			if(app.eng.gd.players[player_num]->to_remove){
				continue;
			}

			int joy_x = app.eng.js.joystates[player_num].lX;
			int joy_y = app.eng.js.joystates[player_num].lY;
			int joy_z = app.eng.js.joystates[player_num].lZ;

			int joy_rx = app.eng.js.joystates[player_num].lRx;
			int joy_ry = app.eng.js.joystates[player_num].lRy;

			int a_angle = get_relative_angle(32767, 32767, joy_x, joy_y);
			int b_angle = get_relative_angle(32767, 32767, joy_rx, joy_ry);

			bool x_is_centered = joy_x > 24000 && joy_x < 40000;
			//bool x_is_centered = joy_x > 10000 && joy_x < 60000;
			bool y_is_centered = joy_y > 24000 && joy_y < 40000;
			bool z_is_centered = joy_z > 24000 && joy_z < 40000;

			bool rx_is_centered = joy_rx > 24000 && joy_rx < 40000;
			bool ry_is_centered = joy_ry > 24000 && joy_ry < 40000;
			...
			if(app.eng.js.joystates[player_num].rgbButtons[0]){
				if(!a_was_pressed){
					a_was_pressed = true;
			...
			if(!z_is_centered && !auto_pilot){
				...
				if(z_up){
					...
					app.eng.gd.players[player_num]->speed = (int)ceil(z_perc * -max_speed);
					app.eng.gd.players[player_num]->move_forward();
				}else if(z_down){
			...
			if(auto_pilot){
				if(app.eng.gd.players[player_num]->speed != 0)
					is_moving = true;
				app.eng.gd.players[player_num]->move_forward();
			}
			...
			if(!x_is_centered || !y_is_centered){
				...
				if(y_up){
					if(app.eng.gd.players[player_num]->active_layer_frames[0] == 0){
						app.eng.gd.players[player_num]->active_layer_frames[0] = 1;
					}else{
						app.eng.gd.players[player_num]->active_layer_frames[0] = 0;
					}
				}

				if(y_down){
					//app.eng.gd.players[player_num]->speed = (int)floor(-5 * y_perc);
					//app.eng.gd.players[player_num]->move_forward();
					if(app.eng.gd.players[player_num]->active_layer_frames[0] == 0){
						app.eng.gd.players[player_num]->active_layer_frames[0] = 1;
					}else{
				...
				if(x_left){
					//app.eng.gd.players[player_num]->sticky_rotate_object_by(-3);
				}
				...
				if(is_moving){
					int change_amt = a_angle - app.eng.gd.players[player_num]->rotations[0];
					...
					app.eng.gd.players[player_num]->rotate_object_by(change_amt);
				}
			}else{ // if(!x_is_centered || !y_is_centered){
				if(is_moving){
					bool pov_press = false;
					if(app.eng.js.joystates[player_num].rgdwPOV[0] < 4200000000){
						pov_press = true;
						a_angle = app.eng.js.joystates[player_num].rgdwPOV[0] / 100;
					}
			...
			if(!rx_is_centered || !ry_is_centered){
				int change_amt = b_angle - app.eng.gd.players[player_num]->rotations[1];
				if((change_amt > -5 && change_amt < 5)){change_amt = 0;}
				if(change_amt > 180) { change_amt = change_amt - 360; }
				if(change_amt < -180) { change_amt = 360 + change_amt; }
				if(change_amt > 5){ change_amt = 5; }
				...
				app.eng.gd.players[player_num]->rotate_layer_by(1, change_amt);
			}

			if(app.eng.js.joystates[player_num].rgbButtons[3]){
			...
			if(app.eng.js.joystates[player_num].rgbButtons[4]){
				app.eng.gd.players[player_num]->fire_projectile();
			}
...
double input_control::get_distance(int x, int y, int x1, int y1){
	int larger_x = x1;
	int smaller_x = x;

	if(larger_x < smaller_x){
		int tmp_x = smaller_x;
		smaller_x = larger_x;
		larger_x = tmp_x;
	}
	...
	if(larger_y < smaller_y){
		int tmp_y = smaller_y;
		smaller_y = larger_y;
		larger_y = tmp_y;
	}

	double x_dist = (larger_x - smaller_x)^2;
	double y_dist = (larger_y - smaller_y)^2;
